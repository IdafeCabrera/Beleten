// frontend/src/features/phrases/components/PhraseModal.tsx
...
import { Phrase, PhraseFormData } from "../../phrases/types/phrase.types";
import "../styles/PhraseModal.css";
...
const PhraseModal: React.FC<PhraseModalProps> = ({
  ...
  const DEFAULT_IMAGE = '../../../assets/images/default-image.jpg'; // Ruta de la imagen por defecto
  ...
  const [imageError, setImageError] = useState(false); // Estado para manejar errores de la imagen

// Función para manejar errores de carga de imagen
const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>) => {
  if (!imageError) {
    console.error("❌ Error al cargar la imagen preview:", e);
    setImageError(true); // Prevenir reintentos
    setImageFile(DEFAULT_IMAGE); // Usar imagen predeterminada
    toastService.error("Error al mostrar la imagen. Mostrando imagen predeterminada.");
  }
};
  ...
  const handleDeleteImage = async () => {
    try {
      ...
      setImageFile(DEFAULT_IMAGE);
      setCurrentPhoto(null);
  
      // Si hay que eliminar la imagen de la base de datos
      if (phrase?.filename) {
        await photoService.deletePhoto(phrase.filename);
  ...
  <div className="ion-padding">
    ...
    <div className="image-preview-container">
      <img
        ...
        alt="Preview"
        onError={handleImageError} // Manejo de errores
        style={{
          maxWidth: "100%",
          maxHeight: "100%",
          objectFit: "contain",
    ...
    <div className="image-actions">
      ...
      <IonButton
        ...
        disabled={!imageFile || imageFile === DEFAULT_IMAGE}
