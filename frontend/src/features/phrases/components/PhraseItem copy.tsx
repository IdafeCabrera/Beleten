// frontend/src/features/phrases/components/PhraseItem copy.tsx
    ...
    return (
      <IonCard className={`custom-card classic-card ${phrase.category?.toLowerCase()}`}>
        ...
        <div className="button-container">
          ...
          <CopyButton text={currentPhrase.text} author={currentPhrase.author} />
        </div>
  
        {/* Contenido del Card */}
        <IonCardHeader>
          <IonCardTitle>{currentPhrase.text}</IonCardTitle>
          {phrase.author && (
            <p className="author">
              — {currentPhrase.author} {currentPhrase.alias ? `(${currentPhrase.alias})` : ""}
            </p>
          )}
          <div className="tags">
            {currentPhrase.tags?.es?.map((tag: string, index: number) => (
